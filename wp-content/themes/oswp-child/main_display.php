...
// is actually marked recurring. ASSUMPTION: combined phrase, not
// separate list items — confirm if a different format is wanted.
if ( get_field( 'is_recurring' ) ) {
	$frequency = get_field( 'recurrence_frequency' );
	$day       = get_field( 'recurrence_day' );
	$phrase    = 'Recurring';
	if ( $frequency ) {
		// stored as "weekly: Weekly" — take the readable half after the colon
...
// Booking required, accessibility — only shown when true (no
// "Booking Required: No" clutter for the common/default case).
...
// ASSUMPTION: is_featured / feature_priority are control-only —
// they drive homepage/billboard selection elsewhere, not shown on
// the event's own page. Confirm if that's wrong.

// Action links — ticket/booking/enquiry URLs. Rendered separately
// as buttons (Forty's ul.actions/button classes), not folded into
// the plain label:value list above, since they're links not text.
$event_actions = array();
$ticket_link = get_field( 'ticket_link' );
if ( $ticket_link ) {
	$event_actions[] = array( 'url' => $ticket_link, 'label' => 'Book A Stall', 'special' => true );
}
...
<section id="one">
	<div class="inner">
		...
		<?php
		// -------------------------------------------------------
		// FEATURED PHOTO — from the featured_image ACF field, not
		// WordPress's native Featured Image (migrated away from
		// core to keep all event content in one ACF panel).
		// -------------------------------------------------------
		...
		<?php
		// -------------------------------------------------------
		// LH/RH BLOCK — same shape as page-duo.php. LH (summary)
		// and RH (details list) each render independently based on
		// whether real data exists for them.
		// -------------------------------------------------------
		if ( $event_summary || ! empty( $event_details ) ) :
		?>
		<div class="row">
		...
		<?php
		// -------------------------------------------------------
		// ACTION LINKS — ticket/RSVP/enquiry buttons. Renders only
		// if at least one link is present.
		// -------------------------------------------------------
